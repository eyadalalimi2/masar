...
$httpMethod = strtoupper((string) ($method ?? 'POST'));
$selectedProduct = $isEdit ? $product : null;
@endphp
...
<form method="POST" action="{{ $formAction }}" enctype="multipart/form-data" class="row g-3" id="productForm">
    @csrf
    @if ($httpMethod !== 'POST')
    @method($httpMethod)
    @endif
    ...
    <div class="col-md-6">
        ...
        <input type="text" name="name" class="form-control" value="{{ old('name', $selectedProduct?->name) }}" required>
    </div>
    <div class="col-md-6">
        <label class="form-label">موديل المنتج</label>
        <input type="text" name="model" class="form-control" value="{{ old('model', $selectedProduct?->model) }}" required>
    </div>
    ...
    <div class="col-md-6">
        ...
        <select name="category_id" class="form-select" required>
            ...
            <option value="{{ $category->id }}"
                ...
                @selected(old('category_id', $selectedProduct?->category_id) == $category->id)>{{ $category->name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="col-md-6 car-models-field" style="display:none;">
        ...
        @php
        $oldCarModels = collect(old('car_models', $selectedProduct?->car_models ?? []))
        ->map(fn($item) => (string) $item)
        ->all();
        @endphp
        <div class="car-models-panel border rounded p-2 bg-light-subtle">
            <div class="d-flex flex-column flex-md-row gap-2 mb-2">
                <input type="text" class="form-control form-control-sm car-model-search" placeholder="ابحث عن موديل السيارة">
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-sm btn-outline-dark car-model-action" data-action="select-all">تحديد الكل</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary car-model-action" data-action="clear">إلغاء التحديد</button>
                </div>
            </div>
            <div class="car-models-list border rounded bg-white p-2" style="max-height: 180px; overflow:auto;">
                ...
                <div class="form-check car-model-item" data-year="{{ $year }}">
                    <input class="form-check-input" type="checkbox" name="car_models[]"
                        id="car-model-{{ $formKey }}-{{ $year }}" value="{{ $year }}"
                        @checked(in_array((string) $year, $oldCarModels, true))>
                    <label class="form-check-label" for="car-model-{{ $formKey }}-{{ $year }}">
                        {{ $year }}
                    </label>
                </div>
                @endforeach
    ...
    <div class="col-md-6">
        ...
        <img src="{{ asset('storage/' . $selectedProduct->image) }}" alt="صورة" class="mt-2"
    ...
    <div class="col-md-6">
        ...
        <select name="status" class="form-select" required>
            <option value="active" @selected(old('status', $selectedProduct?->status ?? 'active') === 'active')>مفعل</option>
            <option value="inactive" @selected(old('status', $selectedProduct?->status) === 'inactive')>معطل</option>
        </select>
    </div>

    <div class="col-12">
        <label class="form-label">الوصف</label>
        <textarea name="description" class="form-control" rows="3">{{ old('description', $selectedProduct?->description) }}</textarea>
    </div>
                    ...
                    <tr class="unit-row">
                        <td>
                            <select name="units[{{ $index }}][unit_id]" class="form-select" required>
                                ...
                                <option value="{{ $unit->id }}" @selected(($row['unit_id'] ?? '' )==$unit->id)>{{ $unit->name }}</option>
                                @endforeach
                            </select>
                        </td>
        ...
        <div class="table-responsive">
            <table class="table table-sm table-bordered align-middle bg-white">
                ...
                <tbody id="variantsTableBody">
                    @php
                    $baseVariants = $selectedProduct
                    ? $selectedProduct->productVariants
                    ->map(fn($variant) => [
                    'variant_type_id' => $variant->variantValue?->variant_type_id,
                    ...
                    $oldVariants = old('variants', $baseVariants);
                    @endphp

                    @foreach ($oldVariants as $index => $row)
                    <tr class="variant-row">
                        <td>
                            <select name="variants[{{ $index }}][variant_type_id]" class="form-select variant-type" data-index="{{ $index }}">
                                <option value="">اختر النوع</option>
                                @foreach ($variantTypes as $type)
                                <option value="{{ $type->id }}" @selected(($row['variant_type_id'] ?? '' )==$type->id)>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                            <select name="variants[{{ $index }}][variant_value_id]" class="form-select variant-value" data-index="{{ $index }}">
...
<input type="hidden" id="productFormUnitsPayload" value="{{ e(json_encode($units, JSON_UNESCAPED_UNICODE)) }}">
<input type="hidden" id="productFormVariantTypesPayload" value="{{ e(json_encode($variantTypesPayload ?? [], JSON_UNESCAPED_UNICODE)) }}">
<input type="hidden" id="productFormOldVariantsPayload" value="{{ e(json_encode($oldVariants, JSON_UNESCAPED_UNICODE)) }}">

<script>
    (() => {
        const units = JSON.parse(document.getElementById('productFormUnitsPayload')?.value || '[]');
        const variantTypes = JSON.parse(document.getElementById('productFormVariantTypesPayload')?.value || '[]');
        const oldVariants = JSON.parse(document.getElementById('productFormOldVariantsPayload')?.value || '[]');

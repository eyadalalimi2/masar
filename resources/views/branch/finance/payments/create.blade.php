...
@section('title', 'تسجيل دفعة للفرع')

@section('content')
    <div class="container-fluid py-2">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="h4 fw-bold mb-0">تسجيل دفعة - {{ $branch->name }}</h1>
            <a href="{{ route('branch.payments.index') }}" class="btn btn-outline-secondary">رجوع</a>
        </div>
        ...
        <form method="POST" action="{{ route('branch.payments.store') }}" class="card border-0 shadow-sm">
            @csrf
            ...
            <div class="card-footer bg-white">
                <button type="submit" class="btn btn-dark">حفظ العملية</button>
            </div>
        </form>
    </div>
@endsection

...
@php
$selectedUnits = $product->productUnits->values();
$selectedVariants = $product->productVariants->values();
@endphp
...
<form method="POST" action="{{ route('branch.products.update', $product) }}" enctype="multipart/form-data"
    class="row g-3" id="productForm">
    @csrf
    @method('PUT')
    <input type="hidden" name="supplier_id" value="{{ $branch->supplier_id }}">
    ...
    <div class="col-md-6 car-models-field" style="display:none;">
        ...
        @php
        $oldCarModels = collect(old('car_models', $product->car_models ?? []))
        ...
        <div class="car-models-list border rounded bg-white p-2" style="max-height: 180px; overflow:auto;">
            ...
            <div class="form-check" data-year="{{ $year }}">
                <input class="form-check-input" type="checkbox" name="car_models[]"
                    id="car-model-branch-edit-{{ $year }}" value="{{ $year }}"
                    @checked(in_array((string) $year, $oldCarModels, true))>
                <label class="form-check-label" for="car-model-branch-edit-{{ $year }}">
                    {{ $year }}
                </label>
            </div>
            @endforeach
        </div>
        <small class="text-muted">مطلوب اختياره عند تصنيف الزيوت.</small>
    </div>
    <div class="col-md-6">
        <label class="form-label">الصورة</label>
        <input type="file" name="image" class="form-control" accept="image/*">
        @if ($product->image)
        <img src="{{ asset('storage/' . $product->image) }}" alt="صورة" class="mt-2"
    ...
    <div class="col-md-6">
        ...
        <select name="status" class="form-select" required>
            <option value="active" @selected(old('status', $product->status) === 'active')>مفعل</option>
            <option value="inactive" @selected(old('status', $product->status) === 'inactive')>معطل</option>
        </select>
    </div>
    <div class="col-12">
            ...
            <table class="table table-sm table-bordered align-middle bg-white">
                ...
                <tbody id="unitsTableBody">
                    ...
                    <tr class="unit-row">
                        ...
                        <td><input type="number" step="0.01" min="0"
                                name="units[{{ $index }}][retail_price]" class="form-control"
                                value="{{ $row['retail_price'] ?? '' }}" required></td>
    ...
    <div class="col-12 d-flex gap-2">
        ...
        <a href="{{ route('branch.products.index') }}" class="btn btn-outline-secondary">إلغاء</a>
    </div>
</form>
...
->values();
$unitsPayload = base64_encode(json_encode($units, JSON_UNESCAPED_UNICODE));
$variantTypesPayloadBase64 = base64_encode(json_encode($variantTypesPayload, JSON_UNESCAPED_UNICODE));
$oldVariantsPayload = base64_encode(json_encode($oldVariants, JSON_UNESCAPED_UNICODE));
@endphp

<div id="branch-products-config" data-units="{{ $unitsPayload }}"
    data-variant-types="{{ $variantTypesPayloadBase64 }}" data-old-variants="{{ $oldVariantsPayload }}" hidden></div>

<script>
    (() => {
        const configNode = document.getElementById('branch-products-config');
        const units = JSON.parse(atob(configNode?.dataset?.units ?? 'W10='));
        const variantTypes = JSON.parse(atob(configNode?.dataset?.variantTypes ?? 'W10='));
        const oldVariants = JSON.parse(atob(configNode?.dataset?.oldVariants ?? 'W10='));
        const categorySelect = document.querySelector('select[name="category_id"]');
        const carModelField = document.querySelector('.car-models-field');
        ...
        function toggleCarModels() {
            ...
            if (carModelField) {
                carModelField.style.display = isOil ? '' : 'none';
            }
        ...
        function reindexUnits() {
            unitsBody.querySelectorAll('.unit-row').forEach((row, index) => {
                ...
                selects[0].setAttribute('name', `units[${index}][unit_id]`);
                inputs[0].setAttribute('name', `units[${index}][wholesale_price]`);
                inputs[1].setAttribute('name', `units[${index}][retail_price]`);
                inputs[2].setAttribute('name', `units[${index}][conversion_factor]`);
            });
        }
        ...
        carModelInputs.forEach((input) => input.addEventListener('change', toggleCarModels));

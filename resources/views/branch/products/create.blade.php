...
<form method="POST" action="{{ route('branch.products.store') }}" enctype="multipart/form-data" class="row g-3"
    id="productForm">
    @csrf
        ...
        <div class="car-models-list border rounded bg-white p-2" style="max-height: 180px; overflow:auto;">
            ...
            <div class="form-check" data-year="{{ $year }}">
                <input class="form-check-input" type="checkbox" name="car_models[]"
                    id="car-model-branch-create-{{ $year }}" value="{{ $year }}"
                    @checked(in_array((string) $year, $oldCarModels, true))>
                <label class="form-check-label" for="car-model-branch-create-{{ $year }}">
                    {{ $year }}
                </label>
            </div>
            @endforeach
        ...
        <div class="table-responsive">
            <table class="table table-sm table-bordered align-middle bg-white">
                ...
                <tbody id="unitsTableBody">
                    @php $oldUnits = old('units', [['unit_id' => '', 'wholesale_price' => '', 'retail_price' => '', 'conversion_factor' => 1]]); @endphp

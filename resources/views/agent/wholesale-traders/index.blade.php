...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('agent.wholesale-traders.create') }}" class="btn btn-dark">إضافة تاجر جملة</a>
</div>
...
<form method="GET" class="card border-0 shadow-sm mb-3">
    <div class="card-body row g-2">
        ...
        <div class="col-md-2 d-grid">
            <a class="btn btn-outline-secondary" href="{{ route('agent.wholesale-traders.index') }}">إعادة</a>
        </div>
    </div>
</form>
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <div class="d-inline-flex gap-1">
                            <a href="{{ route('agent.wholesale-traders.edit', $customer) }}"
                                class="btn btn-sm btn-outline-primary">تعديل</a>
                            <form method="POST"
                                action="{{ route('agent.wholesale-traders.toggleStatus', $customer) }}">
                                @csrf
                                @method('PATCH')
                                <button class="btn btn-sm btn-outline-dark" type="submit">تبديل الحالة</button>
                            </form>
                            <form method="POST" action="{{ route('agent.wholesale-traders.destroy', $customer) }}"

...
@section('content')
    <h1 class="h4 fw-bold mb-4">إضافة فرع</h1>
    ...
    <form action="{{ route('agent.branches.store') }}" method="POST" class="row g-3">
        @csrf
    ...
    <script>
        (() => {
            ...
            window.initAgentBranchCreateMapPicker = function() {
            ...
            script.src =
                `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(mapsKey)}&libraries=places&callback=initAgentBranchCreateMapPicker&language=ar`;
            script.async = true;
            script.defer = true;
            script.onerror = () => {

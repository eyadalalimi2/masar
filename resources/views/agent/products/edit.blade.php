...
<form method="POST" action="{{ route('agent.products.update', $product) }}" enctype="multipart/form-data" class="row g-3"
    id="productForm">
    @csrf
    @method('PUT')
            ...
            <div class="car-models-list border rounded bg-white p-2" style="max-height: 180px; overflow:auto;">
                ...
                <div class="form-check car-model-item" data-year="{{ $year }}">
                    <input class="form-check-input" type="checkbox" name="car_models[]"
                        id="car-model-agent-edit-{{ $year }}" value="{{ $year }}"
                        @checked(in_array((string) $year, $oldCarModels, true))>
                    <label class="form-check-label" for="car-model-agent-edit-{{ $year }}">
                        {{ $year }}
                    </label>
                </div>
                @endforeach

                        ...
                        <tr>
                                    ...
                                    <ul class="mb-0 ps-3">
                                        @foreach ($row->items as $item)
                                            <li>
                                                ...
                                                - {{ number_format((float) $item->requested_quantity, 3) }}
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
                            ...
                            <td>
                                <div class="d-flex flex-wrap gap-2">
                                    <a href="{{ route('agent.replenishment.pdf', $row) }}" target="_blank"
                                    ...
                                    <form method="POST" action="{{ route('agent.replenishment.fulfill', $row) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button class="btn btn-sm btn-primary" {{ $canFulfill ? '' : 'disabled' }}>تزويد
                                            الكل</button>
                                    </form>
                                </div>
                            </td>

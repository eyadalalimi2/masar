...
@section('pdf-body')
    @php
        ...
        $requestedAt = $replenishment->created_at?->format('Y-m-d H:i') ?? '-';
        $resolvedAt = $replenishment->updated_at?->format('Y-m-d H:i') ?? '-';
    @endphp
    ...
    <div class="meta-box">
        <table style="border:0; border-collapse:collapse; width:100%;">
            ...
            <tr>
                ...
                <td style="border:0; padding:2px 0;"><strong>هاتف الفرع:</strong> {{ $replenishment->branch?->phone ?? '-' }}
                </td>
            </tr>
        </table>
    </div>

    <h3>تفاصيل الأصناف المطلوبة</h3>
    <table>
        <thead>
            <tr>
                <th>الموديل</th>
        ...
        <tbody>
            @forelse ($replenishment->items as $item)
                <tr>
                    <td>{{ $item->product?->model ?? '-' }}</td>
                    <td>{{ $item->product?->name ?? '-' }}</td>
                    <td>{{ $item->productUnit?->unit?->name ?? '-' }}</td>
                    <td>{{ number_format((float) ($item->requested_quantity ?? 0), 3) }}</td>
                    <td>{{ number_format((float) ($item->productUnit?->stock_quantity ?? 0), 3) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="5" class="text-center">لا توجد أصناف مرتبطة بهذا الطلب.</td>
                </tr>
            @endforelse
        </tbody>
    </table>

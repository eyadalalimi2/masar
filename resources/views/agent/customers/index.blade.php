...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h4 fw-bold mb-0">العملاء التجاريون</h1>
        <a href="{{ route('agent.customers.create') }}" class="btn btn-dark">إضافة عميل</a>
    </div>
        ...
        <div class="card-body row g-2">
            ...
            <div class="col-md-3">
                <select name="type" class="form-select">
                    ...
                    <option value="workshop" @selected($type === 'workshop')>ورشة</option>
                    <option value="retail_store" @selected($type === 'retail_store')>محل تجزئة</option>
                </select>
            </div>
            <div class="col-md-3">
                ...
                <tbody>
                    @forelse ($customers as $customer)
                        <tr>
                            <td>{{ $customer->id }}</td>
                            <td>{{ $customer->name }}</td>
                            <td>{{ $customer->type === 'workshop' ? 'ورشة' : 'محل تجزئة' }}</td>
                            ...
                            <td class="text-end">
                                <div class="d-inline-flex gap-1">
                                    <a href="{{ route('agent.customers.edit', $customer) }}"
                                        class="btn btn-sm btn-outline-primary">تعديل</a>
                                    <form method="POST" action="{{ route('agent.customers.toggleStatus', $customer) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button class="btn btn-sm btn-outline-dark" type="submit">تبديل الحالة</button>
                                    </form>
                                    <form method="POST" action="{{ route('agent.customers.destroy', $customer) }}"

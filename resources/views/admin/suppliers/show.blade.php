                    ...
                    <div id="supplierAgentInfoCollapse" class="accordion-collapse collapse"
                        ...
                        <div class="accordion-body">
                            <div class="row g-4">
                                ...
                                <div class="col-12">
                                    <div class="fw-semibold text-muted small mb-2">طلبات تعديل الحقول من الوكيل</div>
                                    @if ($supplier->fieldChangeRequests->isEmpty())
                                        <div class="text-muted">لا توجد طلبات تعديل مرسلة.</div>
                                    @else
                                        @php
                                            $fieldLabels = [
                                            ...
                                            <table class="table table-sm align-middle mb-0">
                                                ...
                                                <tbody>
                                                    @foreach ($supplier->fieldChangeRequests as $requestItem)
                                                        <tr>
                                                            <td>{{ $fieldLabels[$requestItem->field_key] ?? $requestItem->field_key }}
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td>
                                                                @if ($requestItem->document_path)
                                                                    <a href="{{ asset('storage/' . $requestItem->document_path) }}"
                                                                        target="_blank" rel="noopener noreferrer"
                                                                        class="btn btn-sm btn-outline-secondary">عرض</a>
                                                                @else
                                                                    -
                                                            ...
                                                            <td>
                                                                @if ($requestItem->status === 'pending')
                                                                    <div class="d-flex gap-1">
                                                                        <form
                                                                            action="{{ route('admin.suppliers.field-change-requests.approve', [$supplier, $requestItem]) }}"
                                                                            method="POST"
                                                                            onsubmit="return confirm('تأكيد قبول طلب تعديل الحقل؟');">
                                                                            @csrf
                                                                            @method('PATCH')
                                                                            <button type="submit"
                                                                                class="btn btn-sm btn-success">قبول</button>
                                                                        </form>
                                                                        <form
                                                                            action="{{ route('admin.suppliers.field-change-requests.reject', [$supplier, $requestItem]) }}"
                                                                            method="POST"
                                                                            onsubmit="return confirm('تأكيد رفض طلب تعديل الحقل؟');">
                                                                            @csrf
                                                                            @method('PATCH')
                                                                            <button type="submit"

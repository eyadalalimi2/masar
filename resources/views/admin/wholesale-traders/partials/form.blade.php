...
<div class="col-md-4">
    <label class="form-label">لوجو تاجر الجملة</label>
    ...
    <a href="{{ $customer->logo_url }}" target="_blank" rel="noopener noreferrer" class="d-inline-block mt-2">
        <img src="{{ $customer->logo_url }}" alt="لوجو تاجر الجملة" class="rounded border"
            style="width: 78px; height: 78px; object-fit: cover;">
    </a>
    @endif
</div>
...
<div class="col-12">
    ...
    <button type="button" id="commercialStoreToggleMapBtn" class="btn btn-sm btn-outline-primary mb-2">تحديد الموقع من
        الخريطة</button>

    <div id="commercialStoreMapPanel" style="display: none;">
        <div id="commercialStoreMapApiHint" class="alert alert-warning py-2 mb-2" style="display: none;"></div>
        <input type="text" id="commercialStoreAddressSearch" class="form-control mb-2"
            placeholder="ابحث عن عنوان أو اسم مكان">
        <div class="d-flex justify-content-end mb-2">
            <button type="button" id="commercialStoreUseMyLocationBtn" class="btn btn-sm btn-outline-primary">تحديد
                موقعي</button>
        </div>
        <div class="alert alert-light border py-2 mb-2 small">
            الإحداثيات الحالية: <span
                id="commercialStoreGpsPreview">{{ old('gps_location', $customer->gps_location ?? '') ?: '-' }}</span>
        </div>
        <div id="commercialStoreMapPicker" class="rounded border" style="height: 320px;"></div>
...
<div class="col-12">
    <label class="form-label">صور تاجر الجملة (متعددة)</label>
    ...
    <div class="d-flex flex-wrap gap-2 mt-2">
        ...
        <div class="d-flex flex-column align-items-center gap-1">
            ...
            @if (isset($customer))
            <button type="button" class="btn btn-sm btn-outline-danger py-0 px-2 js-delete-store-image"
                data-url="{{ route('admin.wholesale-traders.removeImage', ['customer' => $customer, 'imageIndex' => $imageIndex]) }}">
                حذف
            </button>
            @endif
        </div>
...
<script>
    (() => {
        ...
        const toggleMapBtn = document.getElementById('commercialStoreToggleMapBtn');
        const mapPanel = document.getElementById('commercialStoreMapPanel');
        const searchInput = document.getElementById('commercialStoreAddressSearch');
        const useMyLocationBtn = document.getElementById('commercialStoreUseMyLocationBtn');
        const mapContainer = document.getElementById('commercialStoreMapPicker');
        const mapApiHint = document.getElementById('commercialStoreMapApiHint');
        ...
        function submitDeleteImage(deleteUrl) {
        ...
        document.querySelectorAll('.js-delete-store-image').forEach((button) => {
        ...
        window.initCommercialStoreMapPicker = function() {
        ...
        function loadMapScript() {
            ...
            script.src =
                `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(mapsKey)}&libraries=places&callback=initCommercialStoreMapPicker&language=ar`;
            script.async = true;
            script.defer = true;
            script.onerror = () => {

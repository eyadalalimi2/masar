...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h4 fw-bold mb-0">إدارة تجار الجملة</h1>
    <a href="{{ route('admin.wholesale-traders.create') }}" class="btn btn-dark">إضافة تاجر جملة</a>
</div>
...
<form method="GET" class="card border-0 shadow-sm mb-3">
    <div class="card-body row g-2">
        ...
        <div class="col-md-2 d-grid">
            <a class="btn btn-outline-secondary" href="{{ route('admin.wholesale-traders.index') }}">إعادة التعيين</a>
        </div>
    </div>
</form>
                ...
                <tr class="{{ $customer->trashed() ? 'table-warning' : '' }}">
                    ...
                    <td class="text-end">
                        <div class="d-inline-flex gap-1">
                            @if (! $customer->trashed())
                            <a href="{{ route('admin.wholesale-traders.edit', $customer) }}"
                                class="btn btn-sm btn-outline-primary">تعديل</a>
                            <form method="POST"
                                action="{{ route('admin.wholesale-traders.toggleStatus', $customer) }}">
                                @csrf
                                @method('PATCH')
                                <button class="btn btn-sm btn-outline-dark" type="submit">تبديل الحالة</button>
                            </form>
                            <form method="POST"
                                action="{{ route('admin.wholesale-traders.destroy', $customer) }}"
                                onsubmit="return confirm('تأكيد حذف تاجر الجملة؟');">
                                @csrf
                                @method('DELETE')
                            ...
                            @else
                            <form method="POST"
                                action="{{ route('admin.wholesale-traders.restore', $customer->id) }}"
                                onsubmit="return confirm('تأكيد استرجاع تاجر الجملة؟');">
                                @csrf
                                @method('PATCH')
                                <button class="btn btn-sm btn-outline-success" type="submit">استرجاع</button>
                            </form>
                            <form method="POST"
                                action="{{ route('admin.wholesale-traders.forceDestroy', $customer->id) }}"
                                onsubmit="return confirm('سيتم حذف تاجر الجملة نهائيًا. متابعة؟');">
                                @csrf
                                @method('DELETE')

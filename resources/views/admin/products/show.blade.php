...
@section('content')
@php
$carModels = collect($product->car_models ?? [])
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('admin.products.index') }}" class="btn btn-outline-secondary">رجوع</a>
</div>
    ...
    <div class="card-body p-4">
        <div class="row g-4">
            ...
            <div class="col-md-9">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label text-muted mb-1">اسم المنتج</label>
                        <div class="fw-semibold">{{ $product->name }}</div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label text-muted mb-1">الموديل</label>
                        <div class="fw-semibold">{{ $product->model }}</div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label text-muted mb-1">الوكيل</label>
                        <div>{{ $product->supplier?->business_name ?? $product->supplier?->owner_name }}</div>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label text-muted mb-1">التصنيف</label>
                    ...
                    <div class="col-md-6">
                        <label class="form-label text-muted mb-1">موديلات السيارة</label>
                        <div>{{ $carModels->isNotEmpty() ? $carModels->join(' - ') : '-' }}</div>
                    </div>
                    <div class="col-12">
                        <label class="form-label text-muted mb-1">الوصف</label>
            ...
            <table class="table table-bordered align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ number_format((float) $unitRow->retail_price, 2) }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="text-center text-muted">لا توجد وحدات</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>

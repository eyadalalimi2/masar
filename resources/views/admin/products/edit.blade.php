...
<form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data"
    class="card border-0 shadow-sm" id="productForm">
    @csrf
    @method('PUT')
        ...
        <div class="row g-3">
                    ...
                    <div class="car-models-list border rounded bg-white p-2" style="max-height: 180px; overflow:auto;">
                        ...
                        <div class="form-check car-model-item" data-year="{{ $year }}">
                            <input class="form-check-input" type="checkbox" name="car_models[]"
                                id="car-model-admin-edit-{{ $year }}" value="{{ $year }}"
                                @checked(in_array((string) $year, $oldCarModels, true))>
                            <label class="form-check-label" for="car-model-admin-edit-{{ $year }}">
                                {{ $year }}
                            </label>
                        </div>
                        @endforeach
                           ...
                           ->all(),
                            ...
                            <tr class="unit-row">
                                ...
                                <td style="width: 90px;"><button type="button"
                                        class="btn btn-sm btn-outline-danger w-100 remove-unit-row">حذف</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="col-12 mt-3">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-dark" id="addVariantRow">إضافة
                        مواصفة</button>
                </div>
                <div class="table-responsive">
                    <table class="table table-sm table-bordered align-middle">
                        ...
                        <tbody id="variantsTableBody">
                           ...
                           ->all(),
                            ...
                            <tr class="variant-row">
                                ...
                                <td style="width: 90px;"><button type="button"
                                        class="btn btn-sm btn-outline-danger w-100 remove-variant-row">حذف</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
...
<script>
    @php
    $variantTypesPayload = $variantTypes->map(
            fn($type) => [
                'id' => $type->id,
                'name' => $type->name,
        ...
        (() => {
            ...
            function reindexUnits() {
                unitsBody.querySelectorAll('.unit-row').forEach((row, index) => {
                    ...
                    row.querySelectorAll('input')[1].setAttribute('name', `units[${index}][retail_price]`);
                    row.querySelectorAll('input')[2].setAttribute('name',
                        `units[${index}][stock_quantity]`);
                    row.querySelectorAll('input')[3].setAttribute('name',

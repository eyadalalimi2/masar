...
<div class="container-fluid py-2">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            ...
            <p class="text-muted mb-0">إدارة وثائق التوثيق لتاجر الجملة.</p>
        </div>
        <a href="{{ route('customer.profile.index') }}" class="btn btn-outline-secondary btn-sm">رجوع للبروفايل</a>
    </div>
    ...
    <div class="mb-3 d-flex gap-2 align-items-center">
        ...
        @else
        <form action="{{ route('customer.profile.request-verification') }}" method="POST" onsubmit="return confirm('إرسال طلب التوثيق للإدارة؟');">
            @csrf
            @method('PATCH')
    ...
    <form method="POST" action="{{ route('customer.profile.verification.update') }}" enctype="multipart/form-data" class="card border-0 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="card-footer bg-white">
            <button class="btn btn-dark" type="submit">حفظ وثائق التوثيق</button>
        </div>
    </form>
</div>
@endsection
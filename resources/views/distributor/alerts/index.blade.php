...
@section('title', 'كل التنبيهات')

@section('content')
    <div class="container-fluid py-2">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
            <div>
                <h1 class="h4 fw-bold mb-1">تنبيهات المندوب</h1>
                <p class="text-muted mb-0">متابعة الطلبات الجديدة وتحديثاتها.</p>
            </div>
            <div class="d-flex gap-2 align-items-center">
                <span class="badge text-bg-info">غير مقروء: {{ $unreadCount }}</span>
                <form method="POST" action="{{ route('distributor.alerts.mark-all') }}">
                    @csrf
                    @method('PATCH')
        ...
        <form method="GET" action="{{ route('distributor.alerts.index') }}" class="card border-0 shadow-sm mb-3">

...
<div class="hero-box reveal rv1 mb-3">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
        ...
        <a href="{{ route('pos.products.create') }}" class="btn btn-light btn-sm">إضافة منتج</a>
    </div>
</div>
...
@endif
@if ($supplierId === null)
<div class="alert alert-warning">لا يوجد وكيل مرتبط بهذا المحل بعد. يتم الربط تلقائيًا بعد أول عملية توريد/شراء.</div>
@endif
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex gap-2 flex-wrap">
                            <a href="{{ route('pos.products.show', $product) }}" class="btn btn-sm btn-outline-dark">عرض</a>
                            <a href="{{ route('pos.products.edit', $product) }}" class="btn btn-sm btn-outline-primary">تعديل</a>
                            <form method="POST" action="{{ route('pos.products.duplicate', $product) }}" onsubmit="return confirm('هل تريد إنشاء نسخة من هذا المنتج؟');">
                                @csrf
                                <button class="btn btn-sm btn-outline-secondary" type="submit">نسخ</button>
                            </form>
                            <form method="POST" action="{{ route('pos.products.destroy', $product) }}" onsubmit="return confirm('هل أنت متأكد من حذف المنتج؟');">
                                @csrf
                                @method('DELETE')

...
namespace App\Http\Controllers\Finance\Workshop;
...
class WorkshopPaymentMethodController extends Controller
{
    public function index(): View
    {
        ...
        $configuredMethods = PortalPaymentMethod::query()
            ->where('portal_type', 'workshop')
            ->where('portal_id', $portalId)
            ->get()
            ->keyBy('payment_method_id');

        return view('workshop.finance.payment-methods', compact('paymentMethods', 'configuredMethods'));
    }
    ...
    private function resolvePortalId(): int
    {
        ...
        abort_unless($workshop instanceof Workshop, 403);

...
namespace App\Http\Controllers\Finance\Pos;
...
class PosPaymentMethodController extends Controller
{
    public function index(): View
    {
        ...
        $configuredMethods = PortalPaymentMethod::query()
            ->where('portal_type', 'pos')
            ->where('portal_id', $portalId)
            ->get()
            ->keyBy('payment_method_id');

        return view('pos.finance.payment-methods', compact('paymentMethods', 'configuredMethods'));
    }
    ...
    private function resolvePortalId(): int
    {
        ...
        abort_unless($pos instanceof Pos, 403);
